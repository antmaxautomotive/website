<template>
  <nav class="fixed w-full p-6">
    <div class="flex items-end justify-between">
      ...
      <!-- Header logo -->
      <div>
        <img src="~/assets/logo2.png" width="50%" alt="AntMax Automotive Limited">
      </div>
        ...
        <ul class="flex space-x-8 text-sm font-sans text-slate-300">
          <li
            ...
            <nuxt-link
              ...
              <font-awesome-icon :icon="['fas', navBarItem.icon]" /><span class="ml-2">{{ navBarItem.label }}</span></nuxt-link>
          </li>
        </ul>
      </div>
      ...
      <aside class="p-5 transform top-0 left-0 w-64 bg-white fixed h-full overflow-auto ease-in-out transition-all duration-300 z-30" :class="isOpen ? 'translate-x-0' : '-translate-x-full'">
        ...
        <ul class="divide-y font-sans">
          <li
            ...
            <nuxt-link
              :to="navBarItem.path"
              class="my-4 inline-block">{{ navBarItem.label }}</nuxt-link>
          </li>
        </ul>
...
import {useRoute, onMounted, ref, provide} from "@nuxtjs/composition-api"
...
const navBarItems = [
  ...
  { path: "/about", label: "About", icon: "clipboard-question" },
  { path: "/contact", label: "Contact", icon: "file-signature" },
]
...
provide('sideBarState', isOpen.value);
...
const currentRoute = (path: string) => {
  if (path === route.value.fullPath) {
    return "active border-b-2 border-blue-500 pb-1"
  } else {
    return ""
  }
}
